...
class PolicyGradientInverse:
    def __init__(
        ...
        self.ep_obs_tea, self.ep_as_star_tea = [], []
    ...
    def store_transition_tea(self, s, a_star):
        self.ep_obs_tea.append(s)
        self.ep_as_star_tea.append(a_star)
    ...
    def learn_state(self):
        ...
        _, loss_state = self.sess.run(
            [self._train_op_state, self.loss_state], feed_dict={
                self.tf_obs2: np.vstack(self.ep_obs_tea),  # shape=[None, n_obs]
                self.tf_acts_star2: np.array(self.ep_as_star_tea),
            })
